...
pub fn add_token<'x>(
    ...
    let list = match parser.state {
        ...
        AddressState::Comment => &mut parser.comment_tokens,
    };
    ...
    while let Some(ch) = stream.next() {
        match ch {
            ...
            b'\\' if parser.state != AddressState::Name => {
                if parser.token_start > 0 {
                    parser = add_token(parser, stream);
                    if parser.token_end + 1 == stream.get_pos() {
                        parser.add_space = false;
                    }
                }
...
mod tests {
    ...
    fn parse_addresses() {
        let inputs = [
            ...
            ("Mary Smith <mary@x.test>, jdoe@example.org, Who? <one@y.test>\n".to_string(), AddressField::List(AddressList::from([Address::Mailbox(Mailbox { name: Cow::from("Mary Smith"), email: Cow::from("mary@x.test") }), Address::Email(Cow::from("jdoe@example.org")), Address::Mailbox(Mailbox { name: Cow::from("Who?"), email: Cow::from("one@y.test") })]))),
            ("<boss@nil.test>, \"Giant; \\\"Big\\\" Box\" <sysservices@example.net>\n".to_string(), AddressField::List(AddressList::from([Address::Email(Cow::from("boss@nil.test")), Address::Mailbox(Mailbox { name: Cow::from("Giant; \"Big\" Box"), email: Cow::from("sysservices@example.net") })]))),
            ("A Group:Ed Jones <c@a.test>,joe@where.test,John <jdoe@one.test>;\n".to_string(), AddressField::Group(Group { name: Cow::from("A Group"), list: AddressList::from([Address::Mailbox(Mailbox { name: Cow::from("Ed Jones"), email: Cow::from("c@a.test") }), Address::Email(Cow::from("joe@where.test")), Address::Mailbox(Mailbox { name: Cow::from("John"), email: Cow::from("jdoe@one.test") })]) })),
            ...
            ("Joe Q. Public <john.q.public@example.com>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("Joe Q. Public"), email: Cow::from("john.q.public@example.com") }))),
            ("Mary Smith <@node.test:mary@example.net>, , jdoe@test  . example\n".to_string(), AddressField::List(AddressList::from([Address::Mailbox(Mailbox { name: Cow::from("Mary Smith"), email: Cow::from("@node.test:mary@example.net") }), Address::Email(Cow::from("jdoe@test  . example"))]))),
            ("John Doe <jdoe@machine(comment).  example>\n".to_string(), AddressField::Address(Address::MailboxWithComment(MailboxWithComment { name: Cow::from("John Doe"), comment: Cow::from("comment"), email: Cow::from("jdoe@machine.  example") }))),
            ("Mary Smith\n    \n\t<mary@example.net>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("Mary Smith"), email: Cow::from("mary@example.net") }))),
            ("=?US-ASCII*EN?Q?Keith_Moore?= <moore@cs.utk.edu>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("Keith Moore"), email: Cow::from("moore@cs.utk.edu") }))),
            ("John =?US-ASCII*EN?Q?Doe?= <moore@cs.utk.edu>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("John Doe"), email: Cow::from("moore@cs.utk.edu") }))),
            ("=?ISO-8859-1?Q?Keld_J=F8rn_Simonsen?= <keld@dkuug.dk>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("Keld Jørn Simonsen"), email: Cow::from("keld@dkuug.dk") }))),
            ("=?ISO-8859-1?Q?Andr=E9?= Pirard <PIRARD@vm1.ulg.ac.be>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("André Pirard"), email: Cow::from("PIRARD@vm1.ulg.ac.be") }))),
            ("=?ISO-8859-1?Q?Olle_J=E4rnefors?= <ojarnef@admin.kth.se>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("Olle Järnefors"), email: Cow::from("ojarnef@admin.kth.se") }))),
            ("ietf-822@dimacs.rutgers.edu, ojarnef@admin.kth.se\n".to_string(), AddressField::List(AddressList::from([Address::Email(Cow::from("ietf-822@dimacs.rutgers.edu")), Address::Email(Cow::from("ojarnef@admin.kth.se"))]))),
            ("Nathaniel Borenstein <nsb@thumper.bellcore.com>\n    (=?iso-8859-8?b?7eXs+SDv4SDp7Oj08A==?=)\n".to_string(), AddressField::Address(Address::MailboxWithComment(MailboxWithComment { name: Cow::from("Nathaniel Borenstein"), comment: Cow::from("םולש ןב ילטפנ"), email: Cow::from("nsb@thumper.bellcore.com") })),),
            ("Greg Vaudreuil <gvaudre@NRI.Reston.VA.US>, Ned Freed\n      <ned@innosoft.com>, Keith Moore <moore@cs.utk.edu>\n".to_string(), AddressField::List(AddressList::from([Address::Mailbox(Mailbox { name: Cow::from("Greg Vaudreuil"), email: Cow::from("gvaudre@NRI.Reston.VA.US") }), Address::Mailbox(Mailbox { name: Cow::from("Ned Freed"), email: Cow::from("ned@innosoft.com") }), Address::Mailbox(Mailbox { name: Cow::from("Keith Moore"), email: Cow::from("moore@cs.utk.edu") })]))),
            ("=?ISO-8859-1?Q?a?= <test@test.com>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("a"), email: Cow::from("test@test.com") })),),
            ("\"=?ISO-8859-1?Q?a?= b\" <test@test.com>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("a b"), email: Cow::from("test@test.com") }))),
            ("=?ISO-8859-1?Q?a?= =?ISO-8859-1?Q?b?= <test@test.com>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("ab"), email: Cow::from("test@test.com") }))),
            ("=?ISO-8859-1?Q?a?=\n   =?ISO-8859-1?Q?b?= <test@test.com>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("ab"), email: Cow::from("test@test.com") }))),
            ("=?ISO-8859-1?Q?a?= \"=?ISO-8859-2?Q?_b?=\" <test@test.com>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("a b"), email: Cow::from("test@test.com") }))),
            (" <test@test.com>\n".to_string(), AddressField::Address(Address::Email(Cow::from("test@test.com")))),
            ("test@test.com\ninvalid@address.com\n".to_string(), AddressField::Address(Address::Email(Cow::from("test@test.com")))),
            ("\"=?ISO-8859-1?Q =?ISO-8859-1?Q?a?= \\\" =?ISO-8859-1?Q?b?=\" <last@address.com>\n\nbody@content.com".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("=?ISO-8859-1?Q a \" b"), email: Cow::from("last@address.com") }))),
            ("=? <name@domain.com>\n".to_string(), AddressField::Address(Address::Mailbox(Mailbox { name: Cow::from("=?"), email: Cow::from("name@domain.com") }))),
        ];
